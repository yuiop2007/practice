...
<%
  ...
  */
  ...
  int pageSize = 3;  											// 1. 한페이지의 분량을 5건으로 정한다.
...
<div class="container">
	<ul class="pagination justify-content-center">
		<% if(pag != 1) { %>
		     <li class="page-item"><a class="page-link" href="gList.jsp?pag=1">1페이지</a></li>
		<% } %>
		<% if(currentBlock > 0) { %>
					<li class="page-item"><a class="page-link" href="gList.jsp?pag=<%=(currentBlock-1)*blockSize+1 %>">이전블록</a></li>
		<% } %>
		<%
		   for(int i=(currentBlock*blockSize)+1; i<=(currentBlock*blockSize)+blockSize ; i++) {
		  	 ...
		  	 else out.println("<li class='page-item'><a class='page-link' href='gList.jsp?pag="+i+"'>"+i+"</a></li>");
		   }
		%>
		<% if(currentBlock < lastBlock) { %>
					<li class="page-item"><a class="page-link" href="gList.jsp?pag=<%=(currentBlock+1)*blockSize+1%>">다음블록</a></li>
		<% } %>
		<% if(pag != totPage) { %>
		     <li class="page-item"><a class="page-link" href="gList.jsp?pag=<%=totPage%>">마지막페이지</a></li>
		<% } %>
	</ul>
</div>
<!-- 페이징처리 끝 -->
